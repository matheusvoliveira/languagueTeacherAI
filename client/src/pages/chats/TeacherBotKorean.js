...
const TeacherBotKorean = () => {
  const [input, setInput] = useState("");
  const [chatLog, setChatLog] = useState([
    {
      user: "gpt",
      message: "안녕하세요, 저는 당신의 한국어 선생님 Nathan입니다. 오늘 무엇을 도와드릴까요?",
    },
  ]);
  ...
  const saveMessageToFirebase = (uid, message, sender) => {
    const userMessagesRef = firebase.database().ref(`users/${uid}/messages/korean`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (input.trim() && !isSubmitting) {
      ...
      try {
        const response = await fetch(apiUrl("/api/chatgpt/korean"), {
          method: "POST",
          headers: {
...
export default TeacherBotKorean;
