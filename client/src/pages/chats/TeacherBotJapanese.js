...
const TeacherBotJapanese = () => {
  const [input, setInput] = useState("");
  const [chatLog, setChatLog] = useState([
    {
      user: "gpt",
      message: "こんにちは、私はあなたの日本語の先生ナサンです。今日はどうお手伝いしましょうか？",
    },
  ]);
  ...
  const saveMessageToFirebase = (uid, message, sender) => {
    const userMessagesRef = firebase.database().ref(`users/${uid}/messages/japanese`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (input.trim() && !isSubmitting) {
      ...
      try {
        const response = await fetch(apiUrl("/api/chatgpt/japanese"), {
          method: "POST",
          headers: {
...
export default TeacherBotJapanese;
